...
const createNotification = async (req, res) => {
    try {
        const { user_id, title, message, link, is_read } = req.body;
        ...
        const result = await sql`
            INSERT INTO notification (user_id, title, message, link, is_read)
            VALUES (${user_id}, ${title}, ${message}, ${link || null}, ${is_read || false})
...
const getAllNotifications = async (req, res) => {
    try {
        ...
        let query = 'SELECT notification_id, user_id, title, message, link, is_read, created_at FROM notification';
        const conditions = [];
        const values = [];
...
const getNotificationById = async (req, res) => {
    try {
        ...
        const result = await sql`SELECT notification_id, user_id, title, message, link, is_read, created_at FROM notification WHERE notification_id = ${id}`;
...
const updateNotification = async (req, res) => {
    try {
        ...
        const { user_id, title, message, link, is_read } = req.body;
        if (!id || isNaN(id)) return res.status(400).json({ error: 'Invalid notification ID' });
        const current = await sql`SELECT user_id, title, message, link, is_read FROM notification WHERE notification_id = ${id}`;
        ...
        const updatedLink = link !== undefined ? link : current[0].link;
        const updatedIsRead = is_read !== undefined ? is_read : current[0].is_read;

        const result = await sql`
            UPDATE notification SET user_id = ${updatedUserId}, title = ${updatedTitle}, message = ${updatedMessage}, link = ${updatedLink}, is_read = ${updatedIsRead}
